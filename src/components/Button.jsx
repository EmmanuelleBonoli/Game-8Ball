...
let historyTable = []; //création du tableau d'historique (Q/R)

function Button({ answers }) {// on a mis la props answers (tableau de réponse) en paramètres de la fonction Button

    function HistoryID(id, savedQuestion, savedAnswer) {          //Fonction pour créer l'objet Q/R historique avec un ID unique (pour le bien de REACT ;D)
        this.id = id;
                ...
                <div className="historyTable"> {/* zone pour l'historique */}
                    ...
                    <div className="table">
                        {historyTable.map((element) => {
                            {/*  on vient lire le tableau historique pour ensuite le display à l'écran */ }
